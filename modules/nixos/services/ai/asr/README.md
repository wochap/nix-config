...
The speech model is a pluggable backend. The core (chunking, diarization,
speaker assignment, turns, rendering, validation, container and security
wrapper) is shared; a backend adapter under `adapters/` supplies the ASR
model and the word aligner.

## Stack

| Component | Role |
|-----------|------|
| Backend adapter (default `qwen3`) | Speech recognition and word timestamps |
...
Select one with `_custom.services.ai.asr.backend`.

| Backend | Models | CUDA image |
|---------|--------|------------|
| `qwen3` | [Qwen3-ASR-1.7B](https://huggingface.co/Qwen/Qwen3-ASR-1.7B) (~4.7 GB) and Qwen3-ForcedAligner-0.6B | pinned `qwenllm/qwen3-asr` (~14 GB) |

### Adding a backend

1. Create `adapters/<name>/default.nix`, a function of `{ pkgs, lib }` that
   returns:
   - `name`: the backend name.
   - `cudaImage`: upstream CUDA image that already contains the backend's
     Python packages.
   - `pipPackages`: pip requirements installed into the local ROCm image.
...
2. Create the Python adapter. It needs two functions:
   ...
   - `load_aligner(device, dtype)` returns an object whose
     ...
     `end` in seconds relative to the chunk. A backend whose transcriber
     returns timestamps itself would return `None`; the pipeline does not
     support that yet.
3. Add the name to the `backend` enum in `default.nix`.

`test_pipeline.py` runs the whole core against a stub adapter; use it as a
template.
...
| Option | Default | Role |
|--------|---------|------|
| `backend` | `qwen3` | Backend adapter under `adapters/` |
...
| `cuda.image` | backend's `cudaImage` | Upstream CUDA image |
...
`asr-<backend>-rocm` image with the backend's `pipPackages` and
...
and model files persist in `~/.cache/asr/<backend>`. The first invocation also
...
regions, merged turns, and the backend name. Use `--output` and
...
inference script and backend adapter read-only, and the dedicated model
cache. It does not
