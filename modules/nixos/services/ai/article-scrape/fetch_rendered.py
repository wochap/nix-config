...
def browser_executable():
    executable = os.environ.get("ARTICLE_SCRAPE_BROWSER") or os.environ.get(
        "ARTICLE_SCRAPE_BROWSER_DEFAULT"
    )
    if not executable:
        raise RuntimeError(
            "no browser configured; set ARTICLE_SCRAPE_BROWSER to an absolute Chrome executable path"
        )
    if not os.path.isabs(executable):
        raise RuntimeError("ARTICLE_SCRAPE_BROWSER must be an absolute path")
