  ...
  config = lib.mkIf cfg.enable {
    nixpkgs.overlays = [
      (final: prev: {
        dwl = prev.dwl.overrideAttrs (oldAttrs: rec {
          version = "f3ceeb16fd260f84901b3bbadb5dee11e4f2fb2d";
          # install patched dwl
          src = prev.fetchFromGitHub {
            owner = "wochap";
            repo = "dwl";
            rev = version;
            hash = "sha256-f/rux71RvTaKVrdFWYCDb1YL9PwFElBvtVoUhFsTcX4=";
          };
    ...
    _custom.hm = lib.mkIf cfg.isDefault {
      ...
      services.swayidle.timeouts = lib.mkAfter [
        {
          timeout = 180;
          command = ''if ! pgrep swaylock; then chayang -d 5 && wlopm --off "*"; fi'';
