...
let
  ...
  start-tmux =
    pkgs.writeScriptBin "start-tmux" # bash
      ''
      ...
      '';
  start-fbterm =
    pkgs.writeScriptBin "start-fbterm" # bash
      ''
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    console = {
      ...
      # better TTY font
      font = lib.mkDefault "${pkgs.terminus_font}/share/consolefonts/ter-v16n.psf.gz";
    };

    _custom.hm = {
      xdg.configFile = {
        "fbterm/fbtermrc".source = lib._custom.relativeSymlink configDirectory ./dotfiles/fbtermrc;
