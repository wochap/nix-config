...
notify_user() {
  ...
  action=$(notify-send --app-name="Takeshot" --expire-time="$EXPIRE_TIME" --replace-id=699 --app-icon="$thumbnail" --icon="$thumbnail" "Screen shooter" "Screenshot Saved" --action="open=Open" --action="edit=Edit" --action="png=Copy PNG")
...
countdown() {
  for sec in $(seq $1 -1 1); do
    notify-send --app-name="Takeshot" --expire-time=1000 --replace-id=698 --app-icon="accessories-screenshot" --icon="accessories-screenshot" "Taking shot in $sec"
    sleep 1
  done
}
